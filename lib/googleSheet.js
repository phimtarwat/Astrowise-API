...
// F:package | G:e-mail | H:created_at | I:payment_intent_id |
// J:receipt_url | K:paid_at
...
export async function findUser(userId, token) {
  try {
    ...
    return {
      ...
      receipt_url: r[9],
      paid_at: r[10],
      _rowNumber: idx + 2, // ใช้เวลา update
    };
  } catch (err) {
...
// ✏️ อัปเดต quota และ used_count
export async function updateUsage(userId, token, newQuota) {
  try {
    const sheets = await getSheet();

    // โหลดข้อมูลทั้งหมดเพื่อหา row
    const res = await sheets.spreadsheets.values.get({
      spreadsheetId: process.env.SHEET_ID,
      range: RANGE,
    });
    ...
    await sheets.spreadsheets.values.update({
      ...
      requestBody: {
        values: [[String(newQuota), String(currentUsed)]],
      },
    });

    console.log(`✅ updateUsage: row ${rowNumber} → quota=${newQuota}, used=${currentUsed}`);
    return true;
  } catch (err) {
...
// ➕ เพิ่ม user ใหม่ (ตอนชำระเงินสำเร็จ Stripe webhook จะเรียกใช้)
